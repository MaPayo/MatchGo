...
public class MessageController {
	...
	@GetMapping("/")
	@Transactional
	public String startMessagesUser(Model model, HttpSession session,HttpServletRequest request) {
		...
		if (request.getSession().getAttribute("idContact") != null) {
			User u =  entityManager.find(User.class, request.getSession().getAttribute("idContact"));
			all.add(u);
	...
	@GetMapping("/nc/{id}")
	public String newContactMessage(@PathVariable long id, HttpSession session){
		session.setAttribute("idContact",id);
